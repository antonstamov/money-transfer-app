...
class WalletRepository(val databaseService: DatabaseService)(implicit ex: ExecutionContext)
  ...
  def updateByUserId(userId: Int, wallet: WalletRow): Future[Option[WalletRow]] = db.run(Wallet.filter(_.userId === userId).update(wallet)).map{case 0 => None case _ => Some(wallet)}
